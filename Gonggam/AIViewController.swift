...
class AIViewController: ViewController {
  ...
  func showOnMainThread(_ boundingBoxes: [YOLO.Prediction], _ elapsed: CFTimeInterval) {
    DispatchQueue.main.async {
      ...
      self.show(predictions: boundingBoxes)
        ...
        self.aiTimer = (self.count/2 - 1) * 7 / 10
        self.timeLabel.text = String(format: "count = %d", self.aiTimer)
  ...
  func show(predictions: [YOLO.Prediction]) {
    for i in 0..<boundingBoxes.count {
      if i < predictions.count {
        ...
        // Show the bounding box.
          if (labels[prediction.classIndex] == "person" && prediction.score * 100 >= 98.5) {
              if (i==0) {self.count += 1}
    ...
    @IBAction func returnMain(_ sender: Any) {
        ...
        UserModel.shared.myInfo.timer += Double(aiTimer)
        UserModel.shared.myInfo.ai_timer += aiTimer
        self.ref.child("total_time").child(UserModel.shared.myInfo.uid).child("ai_time").setValue(UserModel.shared.myInfo.ai_timer)
